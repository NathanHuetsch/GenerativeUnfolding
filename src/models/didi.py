...
class DirectDiffusion_Padded(DirectDiffusion):
    """
    Class implementing a conditional CFM model
    """
    ...
    def __init__(self, params: dict):
        ...
        assert params["dims_c"] > params["dims_in"], "DiDi_padded only implemented one-sided"
        self.dims_in = params["dims_c"]
        self.dims_c = 0
        self.dims_padding = params["dims_c"] - params["dims_in"]
        self.padding_mode = params.get("padding_mode", "zero")
        self.params["network_params"]["final_output_dim"] = params["dims_c"]
    ...
    def sample(self, x_0: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # return the generated samples
        return x_t[-1][:, :int(self.dims_in-self.dims_padding)]
    ...
    def batch_loss(
        ...
        # Pad x_in appropriately:
        if self.padding_mode == "zero":
            padding = torch.zeros((x_0.size(0), self.dims_padding), dtype=x_0.dtype, device=x_0.device)
        elif self.padding_mode == "normal":
            padding = torch.randn((x_0.size(0), self.dims_padding), dtype=x_0.dtype, device=x_0.device)
        x_1_padded = torch.cat([x_1, padding], dim=-1)

        # Calculate x_t along the trajectory and the derivative x_t_dot
        x_t, x_t_dot = self.trajectory(x_0, x_1_padded, t)
        # Predict the velocity
        t = self.t_embedding(t)
        x_t = self.x_embedding(x_t)
        if self.give_x1:
            ...
            v_pred = self.net(t, x_t)
        # Calculate the loss
        #cfm_loss = self.loss_fct(v_pred[:, :int(self.dims_in-self.dims_padding)], x_t_dot[:, :int(self.dims_in-self.dims_padding)])
        cfm_loss = self.loss_fct(v_pred, x_t_dot)
