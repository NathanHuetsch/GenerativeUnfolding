...
class ParticleINN(nn.Module):
    ...
    def forward(
        ...
        return torch.where(multiplicity_mask, prob_1 + prob_2 + prob_3 + prob_m, zero).sum(dim=1)
...
class Transfermer(nn.Module):
    ...
    def log_prob(self, x: torch.Tensor, c: torch.Tensor) -> torch.Tensor:
        ...
        if not self.bayesian_transfer:
            return self.particle_inn(x, embedding, self.mass_mask, multiplicity_mask)

        else:
            log_probs = []
            for layer in self.bayesian_layers:
                layer.map = True
            log_probs.append(self.particle_inn(x, embedding, self.mass_mask, multiplicity_mask))
