...
class ClassifierPlots(Plots):
    def __init__(
        self,
        observables: list[Observable],
        losses: dict,
        x_test: torch.Tensor,
        classes_test: torch.Tensor,
        classes_predict: torch.Tensor,
        event_type: Optional[torch.Tensor],
    ):
        """
        Initializes the plotting pipeline with the data to be plotted.
        Args:
            ...
            losses: Dictionary with loss terms and learning rate as a function of the epoch
            x_test: Data from test dataset
            classes_test: Classes from test dataset
            classes_predict: Predicted classes
        """
        self.observables = observables
        self.losses = losses
        self.classes_test = classes_test.cpu().numpy()
        self.classes_predict = classes_predict.cpu().numpy()
        self.obs_test = []
        self.bins = []
        for obs in observables:
            o_test = obs.compute(x_test, event_type)
        ...
        self.bayesian = len(self.obs_test[-1].shape) == 3
    ...
    def plot_classes(self, file: str):
        ...
        with PdfPages(file) as pp:
            for obs, bins, data in zip(self.observables, self.bins, self.obs_test):
                binned_classes_test, _, _ = binned_statistic(data, self.classes_test.T, bins=bins)
                binned_classes_predict, _, _ = binned_statistic(data, self.classes_predict.T, bins=bins)
